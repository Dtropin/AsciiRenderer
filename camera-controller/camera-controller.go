...
func (m *CameraController) GetPos() (x, y, z float64) {
	x, y, z = m.cameraState.x, m.cameraState.y, m.cameraState.z
	return
}

func (m *CameraController) SetPos(x, y, z float64) {
	m.cameraState.x = x
	m.cameraState.y = y
	m.cameraState.z = z
}

func (m *CameraController) AdjustPos(dx, dy, dz float64) {
	m.cameraState.x += dx
	m.cameraState.y += dy
	m.cameraState.z += dz
}
