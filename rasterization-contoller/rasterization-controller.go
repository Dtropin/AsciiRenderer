			...
			if len(intersections) == 2 {
				...
				for x := xStart; x <= xEnd; x++ {
					...
					if wp > zbuff[x][y] {
						...
						if x == xStart && mesh.OutlineEdges[intersections[0].edgeKey] == 1 && (intersections[0].k == math.Inf(1) || intersections[0].k == math.Inf(-1)) {
							viewPortController.SetChar(xStart, y, '|')
							continue
						}

						if x == xEnd && mesh.OutlineEdges[intersections[1].edgeKey] == 1 && (intersections[1].k == math.Inf(1) || intersections[1].k == math.Inf(-1)) {
